...
#### Page - Главный контейнер страницы

- Назначение: управляет блокировкой, каталогом и счётчиком товаров.
- Сеттеры:
  - `counter`: обновляет счётчик в хедере.
  - `catalog`: отображает список карточек.
  - `locked`: блокирует/разблокирует интерфейс.
- События: `card:select`, `basket:open`.
